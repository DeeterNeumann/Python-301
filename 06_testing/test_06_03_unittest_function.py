...
class TestMultiply(unittest.TestCase):
    ...
    def test_multiply_positive_numbers(self):
        """Test that multiplying two positive numbers returns the correct product."""
        self.assertEqual(multiply.multiply(4, 5), 20)

    def test_multiply_by_zero(self):
        """Test that multiplying any number by zero does not equal the non-zero factor."""
        self.assertNotEqual(multiply.multiply(0, 10), 10)

    def test_intentional_failure(self):
        """Test is intentionally set to fail."""
        self.assertEqual(multiply.multiply(3, 3), 10)
